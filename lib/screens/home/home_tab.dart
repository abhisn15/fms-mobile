...
class _HomeTabState extends State<HomeTab> with TickerProviderStateMixin, WidgetsBindingObserver {
  ...
  Future<void> _loadMyTeamInfo() async {
    ...
    try {
      ...
      if (!mounted) return;
      final leaderNames = teams
          .map((team) => team.leaderName.trim())
          .where((name) => name.isNotEmpty && name.toLowerCase() != 'tanpa leader')
          .toSet()
          .toList()
        ..sort();
      setState(() {
  ...
  DateTime? _parseCheckInDateTime(AttendanceRecord? today) {
    ...
    try {
      ...
      // Use date from attendance record if available
      DateTime baseDate;
      try {
      ...
      final recordDateOnly = DateTime(baseDate.year, baseDate.month, baseDate.day);

      // Adjust only for true overnight shift (early morning + late check-in time)
      if (recordDateOnly.isAtSameMomentAs(todayDateOnly)) {
        ...
        if (isEarlyMorning && isLateEveningCheckIn) {
          ...
          debugPrint('[HomeTab] Overnight shift: early morning + late check-in, using yesterday date');
        }
      } else if (recordDateOnly.isAfter(todayDateOnly)) {
        ...
        debugPrint('[HomeTab] Future record date detected, clamping check-in date to today');
      }
  ...
  List<DailyShift> _getRemainingShifts(
    ...
    return todayShifts
        .where((shift) => shift.id.isNotEmpty && !completedShiftIds.contains(shift.id))
        .toList();
  }
  ...
  DailyShift? _selectShiftForCheckIn(
    ...
    final availableShifts = _getRemainingShifts(todayShifts, todayRecords)
        .where((shift) => !_isOffShift(shift))
        .toList();
    ...
    availableShifts.sort((a, b) => _effectiveStartMinutes(a, nowMinutes)
  ...
  DateTime? _calculateExpectedShiftEnd(AttendanceRecord? record, DailyShift? shift) {
    ...
    if (checkInDateTime != null) {
      baseDate = DateTime(checkInDateTime.year, checkInDateTime.month, checkInDateTime.day);
    } else {
      try {
    ...
    final startDateTime = baseDate.add(Duration(
      hours: startMinutes ~/ 60,
      minutes: startMinutes % 60,
    ));
    final endDateTime = overnight
        ? startDateTime.add(const Duration(days: 1)).add(Duration(
            hours: endMinutes ~/ 60,
  ...
  List<Map<String, String>> _buildShiftStatusList(
    ...
    if (shifts.isNotEmpty) {
      ...
      for (var i = 0; i < sorted.length; i++) {
        ...
        final hasRecord = record.id.isNotEmpty || record.checkIn != null || record.checkOut != null;
        ...
        final shiftLabel = shift.name.isNotEmpty ? shift.name : (shift.code.isNotEmpty ? shift.code : "Shift");
        items.add({
    ...
    final shiftlessRecords = records.where((r) => r.shiftId == null || r.shiftId!.isEmpty).toList();
    ...
    for (var i = 0; i < shiftlessRecords.length; i++) {
      ...
      items.add({
        "title": shiftlessRecords.length > 1 ? "Tanpa Shift #${i + 1}" : "Tanpa Shift",
        "range": "-",
        "in": checkIn,
        "out": checkOut,
        "status": status,
  ...
  void _syncPersistentNotification(
    ...
    final resolvedShift = _resolveShiftForNotification(record, shiftProvider) ?? fallbackShift;
    final key = '${record.id}|${record.checkIn ?? ''}|${resolvedShift?.id ?? ''}';
    if (_notificationKey == key) return;
    _notificationKey = key;
    PersistentNotificationService.updateCheckInNotification(record, shift: resolvedShift);
    PersistentNotificationService.updateStoredRecord(record, shift: resolvedShift);
  }
    ...
    if (today != null && (today.shiftId == null || today.shiftId!.isEmpty)) {
      ...
      try {
        ...
        recordDate = checkInDateTime != null
            ? DateTime(checkInDateTime.year, checkInDateTime.month, checkInDateTime.day)
            : null;
      }
          ...
          if (success) {
            ...
            if (mounted) {
              ...
              try {
                ...
                debugPrint('[HomeTab] Failed to refresh shifts after check-out: $e');
              }
              if (mounted) {
                setState(() => _selectedShift = null);
              }
  ...
  Widget _buildTodaysAttendanceCard(BuildContext context) {
    return Consumer<AttendanceProvider>(
      builder: (context, attendanceProvider, _) {
        ...
        final completedCount = shiftItems.where((item) => item["status"] == "Selesai").length;
  ...
  Widget _buildShiftSelectionInCard(BuildContext context) {
    return Consumer2<ShiftProvider, AttendanceProvider>(
      builder: (context, shiftProvider, attendanceProvider, _) {
        ...
        final isBlocked = shiftProvider.shiftData?.blocked ?? false;
        final blockedMessage = shiftProvider.shiftData?.blockedMessage ?? 'Anda tidak dapat melakukan absensi hari ini.';
        if (isBlocked) {
          return Container(
            padding: const EdgeInsets.all(12),
            decoration: BoxDecoration(
        ...
        final todayShifts = shiftProvider.todayShifts;
        final remainingShifts =
            _getRemainingShifts(todayShifts, attendanceProvider.todayRecords);
                                      ...
                                      child: Consumer<ShiftProvider>(
                                        builder: (context, shiftProvider, _) {
                                          ...
                                          if (today.shiftId != null) {
                                            ...
                                            DailyShift? shift;
                                            try {
                                              shift = shiftProvider.todayShifts.firstWhere(
