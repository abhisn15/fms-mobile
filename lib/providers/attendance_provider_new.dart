...
class AttendanceProvider with ChangeNotifier {
  ...
  AttendanceRecord? get todayAttendance => _attendanceData?.today.firstOrNull;
  ...
  Future<void> loadAttendance({DateTime? startDate, DateTime? endDate, bool forceRefresh = false}) async {
    ...
    try {
      ...
      if (_attendanceData?.today != null) {
        debugPrint('[AttendanceProvider] Today: ${_attendanceData!.today.first.checkIn} - ${_attendanceData!.today.first.checkOut}');

        // Restore persistent notification if check-in is active
        final today = _attendanceData!.today.first;
